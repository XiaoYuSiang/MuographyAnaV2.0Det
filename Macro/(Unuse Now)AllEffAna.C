...
void AllEffAna(const char *Path_Effsource03, const char *Path_Effsource12, const char *Path_OpRes, const int dataTGap=140){
  // char Path_OpRes[150];
  ...
  int BD[6]={1,3,5,4};
  setTDRStyle();
  ifstream innilo(Form(Path_Effsource12));//"%sGT%ddata_s4_4l_tf_nilo_0.csv",Path_OpRes,dataTGap));
  ifstream inilo (Form(Path_Effsource03));//"%sGT%ddata_s4_4l_tf_ilo_0.csv" ,Path_OpRes,dataTGap));
  ofstream outEff(Form("%sEffOfV2_0AtS4_11FExpGT%d.txt",Path_OpRes,dataTGap));
  double Eff[4][16] = {};
  int bd=0;
  int trash=0;
  ...
  cevent->Print(Form("%sEffAllNoGT%dBothNoise.pdf",Path_OpRes,dataTGap));
  
  char Path_rotfile[100];
  sprintf(Path_rotfile,"%sEffFactorGT%d.root",Path_OpRes,dataTGap);
