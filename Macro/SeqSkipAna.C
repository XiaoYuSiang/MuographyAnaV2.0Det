...
void SeqSkipAna() {
  ...
  for(int i0=0;i0<NumBD;i0++){
    ...
    hSkTModc[i0] -> SetTitle(Form("MTB-%02.f iZ: %d",BD[i0]*1.,i0));
  ...
  for (Long64_t ev = 0; ev < evs ; ++ev) {
    ...
    for(int iH=0; iH<nHits; iH++){
      int tmp_skipNum = (seq[iH] - seqNow[BDINV[board[iH]]][channel[iH]] );
      if(tmp_skipNum != 1  && seqNow[BDINV[board[iH]]][channel[iH]]!=0 ){
        
        hSkT[BDINV[board[iH]]]->Fill(unixtime,channel[iH],tmp_skipNum);
        NumSeqSK[BDINV[board[iH]]][channel[iH]] += tmp_skipNum;
        // cout<<unixtime<<"  B:C  "<<board[iH]<<"  "<<channel[iH]<<" skip  "<<tmp_skipNum<<"  "<<seqNow[BDINV[board[iH]]][channel[iH]]<<" -> "<<seq[iH]<<endl;
        // sleep(1);
      }
      seqNow[BDINV[board[iH]]][channel[iH]] = seq[iH];
  ...
  c1 = new TCanvas("c1","c1",1080*4,1080*4);
  c1->Divide(2,2);
  c1->cd();
  for(int i0=0;i0<NumBD;i0++){
    c1->cd(i0+1);
    hSKN[i0] = new TH1F(Form("hSKN%02.f",i0*1.),Form("MTB%02.f",i0*1.),NumCh,0,NumCh);
    hSKN[i0]->SetLineColor(ColorArr9[i0]);
